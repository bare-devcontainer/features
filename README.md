...
| [mise](src/mise)<br>`ghcr.io/bare-devcontainer/features/mise` | mise, the polyglot tool and runtime manager, for the toolchains and CLIs a project pins itself. The tools it installs and its download cache live on volumes, so a rebuild does not reinstall them. | [GitHub Releases](https://github.com/jdx/mise/releases). Upstream's signed checksum file is committed here, so the install verifies the download with `sha256sum` alone and CI checks that file's minisign signature. |
...
upstream's own checksum file with the signature it published for it. Each Feature's README
shows how to inspect what it verifies against.

**Where verification happens.** Wherever it costs a container nothing. A Feature whose
upstream signs its checksums ships those checksums rather than the tooling to check a
signature, so the install verifies the download with `sha256sum` and installs no package in
order to do it; the `CI` workflow verifies the committed checksums against upstream's
signature on every change instead
...
[`scripts/pin-checksums.sh`](scripts/pin-checksums.sh), which writes what it downloaded
only once upstream's signature over it verifies.
